...
This is my "missing grades" list of how many NA values are in each category -- very valuable since D2 has a lot of missing data.
...
notbad_and_ok_data <- filter(missing_grades_data, grade %in% c("NotBad", "OK", "Good")) #This will keep the good variables where, relatively speaking, a minimium of NAs are quite low. Now I want to see if the decision tree algorithm will remove the NAs and plot the data. 
notbad_and_ok_data
```
